...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="testStudent/showList.do" method="post" name="Form" id="Form">
			<table>
				<tr>
					<td>
...
<%--											<button title="现金缴费" class="btn btn-mini btn-info" data-toggle="dropdown" onclick="payCash('${var.ID}','${var.NAME}','${var.STD_NUM}')">缴费</button>--%>
											<button title="现金缴费" class="btn btn-mini btn-info" data-toggle="dropdown" onclick="top.siMenu('${var.ID}','${var.ID}','${var.NAME}-缴费管理','paycash/listByStdId.do?STD_ID=${var.ID}')">缴费</button>
...
<%--					</c:if>--%>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>testStudent/goAdd.do';
			 diag.Width = 450;
			 diag.Height = 655;
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>testStudent/delete.do?ID="+Id+"&tm="+new Date().getTime();
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>testStudent/goEdit.do?ID='+Id;
			 diag.Width = 450;
			 diag.Height = 655;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		function payCash(Id,stdName,stdNum){
			 ...
			 diag.Drag=true;
			 diag.Title ="现金缴费";
			 diag.URL = '<%=basePath%>paycash/goPayCash.do?STD_ID='+Id;
			 diag.Width = 252;
			 diag.Height = 255;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
