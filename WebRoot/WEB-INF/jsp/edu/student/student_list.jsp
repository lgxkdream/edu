	...
	<div class="row-fluid">
			...
			<table>
				<tr>
					...
					<td style="vertical-align:top;">
					<input type="hidden" name="STATUS" value="<%=Dic.STD_STATUS_ZHENGSHI %>">
						...
						<c:forEach items="${varList}" var="var" varStatus="vs">
							<tr>
								<td class='center' style="width: 30px;">
									...
									<input type='hidden' name='emails' value="${var.EMAIL}" />
									<input type='hidden' name='stunames' value="${var.NAME}" />
								</td>
								...
								<td style="width: 30px;" >
									<div class="hidden-phone visible-desktop btn-group">
										...
										<button title="录入卡号" class="btn btn-mini btn-info" data-toggle="dropdown" onclick="editCardNo('${var.ID}');">录入卡号</button>
										<br/>
										<button title="贷款" class="btn btn-mini btn-info" data-toggle="dropdown" onclick="top.siMenu('${var.ID}','${var.ID}','${var.NAME}-贷款','payloan/listByStdId.do?STD_ID=${var.ID}')">贷款</button>
										<button title="查看出勤" class="btn btn-mini btn-success" data-toggle="dropdown" onclick="top.siMenu('${var.ID}','${var.ID}','${var.NAME}-查看出勤','student_attendance/listByStudentCard.do?ID=${var.ID}')">查看出勤</button>
										<button title="重置密码" class="btn btn-mini btn-info" data-toggle="dropdown" onclick="resetPassword('${var.ID}')">重置密码</button>
									</div>
								</td>
							</tr>
...
<%--					</c:if>--%>
						...
						<a class="btn btn-small btn-yellow" onclick="doEmail();">发送邮件</a>
				</td>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		//修改
		function editCardNo(Id){
			 top.jzts();
			 var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>student/goEditCardNo.do?ID='+Id;
			 diag.Width = 450;
			 diag.Height = 155;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
...
<%--						}--%>
		...
		function doEmail(){
			...
			{
				  if(document.getElementsByName('ids')[i].checked){
				  	if(str=='') str += document.getElementsByName('emails')[i].value;
				  	else str += ';' + document.getElementsByName('emails')[i].value;
				  	if(str_name=='') str_name += document.getElementsByName('stunames')[i].value;
				  	else str_name += ';' + document.getElementsByName('stunames')[i].value;
				  }
			}
			...
			if(str==''){
				...
				top.jzts();
				 ...
				 diag.Drag=true;
				 diag.Title ="发送电子邮件";
				 diag.URL = '<%=basePath%>head/goSendEmail1.do?EMAIL='+str+'&STUNAMES='+str_name;
				 diag.Width = 660;
				 diag.Height = 470;
				 diag.CancelEvent = function(){ //关闭事件
					diag.close();
		...
		function resetPassword(id){
			//'student/resetPassword.do?ID=${var.ID}'
			 bootbox.confirm('确定要重置密码？', function(result) {
				if(result) {
					$.ajax({   
			           url:'<%=basePath%>student/resetPassword.do?ID='+id,   
			           type:'post',   
			           cache: false,
			           dataType:'json',
			           success:function(data){   
