	...
	<form action="student/${msg }.do" name="Form" id="Form" method="post" class="form-horizontal" >
		...
		<table>
			...
			<tr>
				...
				<c:choose >
				    <c:when test='${pd.ID==null || pd.ID==""}'>
						  <option value='${data.ID}' <c:if test='${data.ID==STD_STATUS_YIXIANG}'>selected</c:if>>${data.NAME}</option>
					</c:when>
				    <c:otherwise >
			...
			<tr>
				<td>
				<input type="text" name="CARD_NO" id="CARD_NO" value="${pd.CARD_NO}" maxlength="32" placeholder="这里输入签到卡号" title="签到卡号"/></td>
			</tr>
			<tr>
				<td>
