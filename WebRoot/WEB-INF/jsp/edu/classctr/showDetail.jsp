...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
		<form action="showClassAttendance/showDetail.do?" method="post" name="Form" id="Form">
					...
					<c:when test="${not empty varList}">
						...
						<c:forEach items="${varList}" var="var" varStatus="vs">
							<tr>
										...
										<td>${var.NAME}</td>
										<td>${var.INPUT_TIME}</td>
										<td></td>
										<td>
											<c:choose>
												<c:when test="${var.TYPE == 'a'}">
												缺勤
												</c:when>
												<c:when test="${var.TYPE == 'l'}">
												迟到
												</c:when>
												<c:when test="${var.TYPE == 'q'}">
												请假
												</c:when>
												<c:when test="${var.TYPE == 't'}">
												正常
												</c:when>
												<c:otherwise>
												不详
...
<%--					<a class="btn btn-small btn-success" onclick="attendance();">考勤录入</a>--%>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>showClassAttendance/goEdit.do?STUDENT_ATTENDANCE_ID='+Id;
			 diag.Width = 450;
			 diag.Height = 355;
			 diag.CancelEvent = function(){ //关闭事件
				 setTimeout("location.reload()",100);
