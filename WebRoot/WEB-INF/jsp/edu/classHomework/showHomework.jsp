  ...
  <div class="row-fluid">
						...
						<c:forEach items="${varList}" var="var" varStatus="vs">
							...
							<td class='center' style="width: 30px;">${vs.index+1}</td>
								<td class='center' style="width: 30px;">${var.TITLE}</td>
										...
										<td>&nbsp;
										<fmt:formatNumber value="${var.COUNT/countStudent*100}" pattern="##.##" minFractionDigits="2"/>%
										</td>
								<td style="width: 30px;" class="center">
									<div class='hidden-phone visible-desktop btn-group'>
										...
										<button title="作业评分" class="btn btn-mini btn-info" data-toggle="dropdown" onclick="top.siMenu('${var.ID}','${var.ID}','${classInfo.NAME}-${fn:substring(var.TITLE, 0, 5) }-作业评分','classHomework/showByCHID.do?CH_ID=${var.ID}')">作业评分</button>
										<button title="查看作业详细" class="btn btn-mini btn-info" data-toggle="dropdown" onclick="top.siMenu('${var.ID}','${var.ID}','${var.TITLE}-查看作业详细','classHomework/HomeworkDetailed.do?HOMEWORK_ID=${var.ID}')">查看作业详细</button>
									</div>
								</td>
							</tr>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function assignHom(id){
			 ...
			 diag.Drag=true;
			 diag.Title ="分配作业";
			 diag.URL = '<%=basePath%>classHomework/assignHw.do?ID='+id;
			 diag.Width = 800;
			 diag.Height = 555;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
