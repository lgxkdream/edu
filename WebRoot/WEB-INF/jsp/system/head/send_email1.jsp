...
<html lang="en">
	<head>
		...
		<!--引入属于此页面的js -->
		<script type="text/javascript" src="static/js/myjs/headEmail.js?tm="+new Date().getTime()></script>
...
<body>
		...
		<input type="hidden" name="STUNAMES" id="STUNAMES" value="${pd.STUNAMES}"/> 
		<table style="width:98%;" >
			...
			<tr>
				<td style="text-align: center;">
					<a class="btn btn-mini btn-primary" onclick="sendEmail_();">发送</a> 
					...
					<label style="float:left;padding-left: 15px;"><input name="form-field-checkbox" class="ace-checkbox-2" type="checkbox" id="allusers" onclick="isAll();" /><span class="lbl">启用模版</span></label>
					<label style="float:left;padding-left: 15px;">
					    <select name="email_model" id="email_model" data-placeholder="请选择模版" style="vertical-align:top;width: 120px;display:none">
							<option value=""> 全部邮件模版</option>
							<c:forEach var="data" items="${dic_emailmodel_list}">
						         <option value='${data.EMAILNAME}_${data.EMAILCONTENT}'>${data.EMAILNAME}</option>
						    </c:forEach>
					  	</select>    
					</label>
				</td>
		...
		<script>$(document).ready(function(){
			//alert('${pd.STUNAMES}');
			//监听select值更改 added by gaobo
			$('#email_model').change(function(){
				//$(this).attr('myattr').val()
				//$(':hidden[name=MODEL]').val($(this).children('option:selected').val());
				var _temp =$(this).children('option:selected').val();
				var array = new Array();
				array=_temp.split("_"); //字符分割 
                $('#TITLE').val(array[0]);
                var ue = UE.getEditor('editor'); 
                ue.setContent(array[1]);
				});
			//复选框点击事件  added by gaobo
			$('#allusers').click(function(){if($('#allusers').is(':checked')){$('#email_model').attr("style","display:block;");}else{$('#email_model').attr("style","display:none;");}});
			})
		</script>
</body>
</html>