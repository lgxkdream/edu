...
//			pd.put("NUM", "");	//收据号
	...
	public ModelAndView goAdd(){
		...
		try {
			...
			paraStdPd.put("ID", pd.get("STD_ID"));
			PageData stdPd=studentService.findById(paraStdPd);
			
			PageData resultPd=new PageData();
			resultPd.put("STD_ID", stdPd.get("ID"));
			resultPd.put("NAME", stdPd.get("NAME"));
			resultPd.put("STD_NUM", stdPd.get("STD_NUM"));
			...
			//数据字典
			int status=Integer.parseInt(stdPd.get("STATUS").toString());
//			List payTypeList=new ArrayList();
			/*
			 * 如果是意向学员，仅显示报名费
			 */
//			if(status==Dic.STD_STATUS_YIXIANG){
//				for(Object obj:Dic.dic_pay_type_list){
//					Map tempPd=(Map)obj;
//					int id=Integer.parseInt(tempPd.get("ID").toString());
//					if(id==Dic.DIC_PAY_TYPE_BAOMINGFEI){
//						payTypeList.add(tempPd);
//					}
//				}
//			}else{
//				for(Object obj:Dic.dic_pay_type_list){
...
//			}
	...
	public ModelAndView goPayCash(){
		...
		try {
			...
			paraPd.put("ID", pd.get("STD_ID"));
			PageData stdPd=studentService.findById(paraPd);
			
			mv.setViewName("edu/paycash/paycash_edit");
			mv.addObject("msg", "payCash");
			mv.addObject("pd", stdPd);
