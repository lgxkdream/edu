...
import com.edu.service.classHomework.ClassHomeworkService;
...
//	}
	...
	public ModelAndView HomeworkDetailed(Page page) {
		...
		try {
			...
			mv.setViewName("edu/classHomework/homeworkDetailed");
			mv.addObject("varList", varList);
			mv.addObject("pd", pd);
			mv.addObject(Const.SESSION_QX, this.getHC()); // 按钮权限
		} catch (Exception e) {
...
//	}
	...
	@RequestMapping(value = "/assignHw")
	public ModelAndView assignHw(Page page) throws UnsupportedEncodingException {
		...
		try {
			PageData classPd = classService.findById(pd);
		...
		mv.addObject("pd", pd);
		mv.setViewName("edu/classHomework/outer");
		return mv;
	...
	public ModelAndView HomeworkTemplates(Page page) {
		...
		try {
			...
			List<PageData> varList = classHomeworkService
		...
		mv.setViewName("edu/classHomework/inner");
		return mv;
	}
	...
	public ModelAndView saveHomework() throws Exception {
		...
		String endTime=pd.getString("END_TIME")+" "+pd.getString("HOUR")+":"+pd.getString("MINUTE")+":00";
		parapd.put("END_TIME", endTime);
		...
		classHomeworkService.saveHomeworkToStudent(parapd);
		mv.addObject("msg", "success");
		mv.setViewName("save_result");
		return mv;
	}
	 ...
	 * @return
	 */
	@RequestMapping(value="showByCHID")
	public ModelAndView showByCHID(){
		ModelAndView mv=new ModelAndView("edu/classHomework/showByCHID");
		PageData pd=getPageData();
		try {
			List list=studentHomeworkService.showStudentHomeworkByCHID(pd);
			mv.addObject("varList", list);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	public ModelAndView goScore(){
		ModelAndView mv=new ModelAndView("edu/classHomework/setScore");
		PageData pd=getPageData();
		try {
