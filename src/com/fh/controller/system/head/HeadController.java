...
public class HeadController extends BaseController {
	...
	@RequestMapping(value="/goSendEmail1",produces = "text/html;charset=UTF-8")
	public ModelAndView goSendEmail1() throws Exception{
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		mv.setViewName("system/head/send_email1");
		mv.addObject("pd", pd);
		mv.addObject("dic_emailmodel_list", Dic.dic_emailmodel_list);
		return mv;
	}
	/**
	 * 发送电子邮件
	...
	public Object sendEmail1(){
		...
		String toStuName = pd.getString("STUNAMES"); //同学姓名
		...
		if(null != strEMAIL && !"".equals(strEMAIL)){
			...
			if(strEM.length == 4){
				if("yes".endsWith(isAll)){
					...
					toEMAIL = toEMAIL.replaceAll(" ", "");
					toStuName = toStuName.replaceAll("；", ";");
					toStuName = toStuName.replaceAll(" ", "");
					String[] arrTITLE = toEMAIL.split(";");
					String[] arrStu = toStuName.split(";");
					zcount = arrTITLE.length;
					try {
						for(int i=0;i<arrTITLE.length;i++){
							if(Tools.checkEmail(arrTITLE[i])){		//邮箱格式不对就跳过
								...
								SimpleMailSender.sendEmail(strEM[0], strEM[1], strEM[2], strEM[3], arrTITLE[i], TITLE, CONTENT.replace("[Stuname]", arrStu[i]), TYPE);//调用发送邮件函数
								count++;
							}else{
								continue;
							}
